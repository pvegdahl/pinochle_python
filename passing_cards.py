...
class PassingCards(NamedTuple):
    ...
    def pass_cards(self, source: str, destination: str, cards: Tuple[Card, Card, Card, Card]) -> "PassingCards":
        ...
        if pass_direction == PassDirection.BID_WINNER_TO_PARTNER:
            ...
            new_partner_hand = self._remove_cards_from_hand(initial_hand=self.partner_hand, cards_to_remove=cards)
        else:
            new_winner_hand = self._remove_cards_from_hand(initial_hand=self.bid_winner_hand, cards_to_remove=cards)
    ...
    def _remove_cards_from_hand(
        ...
        return cls._remove_cards_from_hand(initial_hand=new_hand, cards_to_remove=cards_to_remove[1:])
